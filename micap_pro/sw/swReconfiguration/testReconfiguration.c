...
*******************************************************************************************************
*
* file: testReconfiguration.c
*
* Test software for parameterized configuration
* This is a main function for testing parameterized 32-bit EXOR  gate
...
void test(XGpio* gpio_InstancePtr, XAxiDma* dma_InstancePtr) {
    ...
    for(i=0; i<numMask; i++) {
    	...
    	micap_reconfigure(gpio_InstancePtr, dma_InstancePtr, output, location_array[0]);
...
void manual_test(XGpio* gpio_InstancePtr, XAxiDma* dma_InstancePtr) {
	u32 input = 0;
    u32 mask = 0;
	u8 parameter[NUMBER_OF_PARAMETERS];
	u8 output[NUMBER_OF_TLUTS_PER_INSTANCE][LUT_CONFIG_WIDTH];
	...
	xil_printf("Reading output register: x%08x (x%08x expected)\n\r\n\r",
	    Xil_In32(XPAR_AXI_XOR_0_BASEADDR+4),
	    (~input^mask));
	xil_printf("Manual test complete!\n\r\n\r");
...
int main(void) {
	...
	u8 user_test = 0; /*User Switch for 0-Manual/1-Automated test */
	...
	status = XGpio_Initialize(&axi_gpio, XPAR_GPIO_SW_DEVICE_ID);
	if (status != XST_SUCCESS)
	{
	...
	XGpio_DiscreteWrite(&axi_gpio, 1, 0x0); // Put data source back in reset state for first run
	...
	if(user_test == 0){
		manual_test(&axi_gpio, &axi_dma);
	}
	else{
		test(&axi_gpio, &axi_dma);
	}
